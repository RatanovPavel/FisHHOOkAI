...
def check_server_permission(token: str, task_id: str) -> bool:
    ...
    if token == "DEMO_TOKEN" or token.startswith("LIFETIME_"):
        print(f" [FishHookAI]: Лицензия подтверждена сервером. Доступ к GPU разрешен!")
        return True
    return False
...
def run_dual_ai_pipeline(prompt_style: str, task_id: str):
    ...
    print(f" [GPU ИИ]: Инициализация процессора: {device.upper()}")
    ...
    try:
        ...
        print(f" [Экран]: Шаг 1 – Созданный товар:")
    ...
    del txt2img_pipe
    gc.collect() # Принудительно очищаем операционку Linux от остатков модели
    if device == "cuda":
        torch.cuda.empty_cache()
    print(" [Лог]: Оперативная память успешно очищена под ноль.")
    
    # --- ЭТАП 2: СОЗДАНИЕ ПРАВИЛЬНОЙ МАСКИ (Товар = Черный, Фон = Белый) ---
    print("\n [ИИ Этап 2]: Запуск rembg. Вырезаю фон и создаю маску для прорисовки рук...")
    ...
    try:
        from IPython.display import display
        print(f" [Экран]: Шаг 2 – Правильная маска для генерации рук:")
        display(mask_image)
    except Exception:
        pass
    ...
    try:
        from IPython.display import display
        print(f" [Экран]: Шаг 3 – Финальный результат (Кейс в руках человека):")
        display(final_image)
    except Exception:
        pass
